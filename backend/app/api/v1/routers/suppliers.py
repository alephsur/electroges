...
async def get_supplier(supplier_id: uuid.UUID, db: DbSession, _: CurrentUser):
    """Return a single supplier by ID."""
    return await SupplierService(db).get_supplier(supplier_id)


@router.post("", response_model=SupplierResponse, status_code=status.HTTP_201_CREATED)
async def create_supplier(data: SupplierCreate, db: DbSession, _: CurrentUser):
...
async def update_supplier(
    supplier_id: uuid.UUID, data: SupplierUpdate, db: DbSession, _: CurrentUser
):
...
async def deactivate_supplier(supplier_id: uuid.UUID, db: DbSession, _: CurrentUser):
    """Soft-delete a supplier (sets is_active = False)."""
    await SupplierService(db).deactivate_supplier(supplier_id)
...
async def list_supplier_inventory_items(
    ...
    return await InventoryItemService(db).list_for_supplier(
...
async def create_supplier_inventory_item(
    ...
    return await InventoryItemService(db).create_for_supplier(supplier_id, data)
...
async def list_supplier_purchase_orders(
    ...
    return await PurchaseOrderService(db).list_by_supplier(
...
async def get_purchase_order(
    ...
    return await PurchaseOrderService(db).get_order(order_id)
...
async def update_purchase_order(
    ...
    return await PurchaseOrderService(db).update_order(order_id, data)
...
async def receive_purchase_order(
    ...
    return await PurchaseOrderService(db).receive_order(order_id)
...
async def cancel_purchase_order(
    ...
    return await PurchaseOrderService(db).cancel_order(order_id)
